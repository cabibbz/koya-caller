...
export type PHICategory =
  ...
  | 'provider_name' | 'facility_name' | 'appointment_date'
  | 'lab_results' | 'genetic_info' | 'biometric';

export type ConsentType =
  | 'voice_recording' | 'ai_processing' | 'data_storage'
  | 'data_sharing' | 'marketing' | 'research';

export type ConsentMethod = 'verbal' | 'written' | 'electronic' | 'implied';

export type PHIAuditEventType =
...
export type PHIAuditOutcome = 'success' | 'denied' | 'error';

export type HealthcareCategory =
  | 'dental' | 'medical' | 'mental_health' | 'veterinary'
  | 'optometry' | 'physical_therapy' | 'chiropractic' | 'other';

export interface ComplianceSettings {
  businessId: string;
  hipaaEnabled: boolean;
  hipaaBaaSignedAt: string | null;
  hipaaBaaSignatoryName: string | null;
  hipaaBaaSignatoryEmail: string | null;
  hipaaBaaDocumentUrl: string | null;
  phiHandlingEnabled: boolean;
  phiInTranscripts: boolean;
  phiInRecordings: boolean;
...
export interface PHIAuditEntry {
  ...
  phiCategoriesAccessed: PHICategory[] | null;
...
export interface CallWithPHI extends Call {
  direction: CallDirection;
  callType: CallType;
  campaignId: string | null;
  queueItemId: string | null;
  containsPhi: boolean;
  phiCategories: PHICategory[] | null;
  phiReviewedAt: string | null;
  phiReviewedBy: string | null;
  recordingEncrypted: boolean;
  recordingEncryptionKeyId: string | null;
}
...
export interface PHIAuditLogRequest {
  eventType: PHIAuditEventType;
  resourceType: string;
  resourceId?: string;
  action: string;
  justification?: string;
  phiCategoriesAccessed?: PHICategory[];
  outcome: PHIAuditOutcome;
  metadata?: Record<string, unknown>;
}
...
// Type guard helpers for Phase 3 features
export function isCallWithPHI(call: Call | CallWithPHI): call is CallWithPHI {
  return 'containsPhi' in call && 'direction' in call;
}
...
export interface ComplianceReport {
  ...
  recordingsWithPHI: number;
  transcriptsWithPHI: number;
  pendingRetentionDeletions: number;
}
