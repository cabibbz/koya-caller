...
export async function fetchCallerContext(
  ...
  try {
    ...
    const { data: calls } = await supabase
      .from("calls")
      .select("id, caller_number, outcome, created_at, sentiment_detected")
      .eq("business_id", businessId)
    ...
    if (calls && calls.length > 0) {
      ...
      const hadNegativeExperience = callHistory.some(
        c => c.sentiment_detected === "frustrated" ||
             c.sentiment_detected === "upset" ||
             c.sentiment_detected === "angry"
      );
      ...
      return {
        ...
        sentiment: {
          previousSentiment: callHistory[0]?.sentiment_detected || undefined,
