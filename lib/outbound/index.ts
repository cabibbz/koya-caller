...
import { checkHIPAAEnabled, checkPatientConsent, logPHIAccess } from "@/lib/hipaa";
...
export async function initiateOutboundCall(
  ...
  // Check HIPAA consent requirements
  const hipaaEnabled = await checkHIPAAEnabled(businessId);
  if (hipaaEnabled) {
    const requiredConsents: Array<"voice_recording" | "ai_processing"> = ["voice_recording", "ai_processing"];
    const missingConsents: string[] = [];

    for (const consentType of requiredConsents) {
      const hasConsent = await checkPatientConsent(businessId, normalizedTo, consentType);
      if (!hasConsent) {
    ...
    if (missingConsents.length > 0) {
      // Log the consent check failure in the audit trail
      await logPHIAccess({
        businessId,
        userId: "system",
        eventType: "phi_access",
        resourceType: "contact",
        resourceId: normalizedTo,
        action: "outbound_call_blocked_consent",
        metadata: {
          phone_number_masked: `***${normalizedTo.slice(-4)}`,
          missing_consents: missingConsents,
          call_purpose: options.purpose,
        },
      });

      return {
        success: false,
        error: `Patient consent required for: ${missingConsents.join(", ")}`,
    ...
    await logPHIAccess({
      ...
      resourceType: "contact",
      resourceId: normalizedTo,
      action: "outbound_call_consent_verified",
      metadata: {
        phone_number_masked: `***${normalizedTo.slice(-4)}`,
        consents_verified: requiredConsents,
        call_purpose: options.purpose,
      },
    });
  }

  // Get business data and phone number
  const [businessResult, phoneResult, aiConfigResult] = await Promise.all([
    supabase
      .from("businesses")
