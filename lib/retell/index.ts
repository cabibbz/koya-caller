...
// =============================================================================
// HIPAA PHI Detection in Call Transcripts
// =============================================================================
...
/**
 * Analyze call transcript for PHI content
 *
 * This function should be called after a call ends to detect
 * any PHI in the transcript. If PHI is detected and auto-detection
 * is enabled for the business, the call is automatically flagged.
 *
 * @param callId - Call ID to analyze
 * @param businessId - Business ID for settings lookup
 * @param transcript - Call transcript to analyze
 * @returns Detected PHI categories or null if analysis skipped
 */
export async function analyzeCallForPHI(
  callId: string,
  businessId: string,
  transcript: Array<{ role: string; content: string }> | string | null
): Promise<PHICategory[] | null> {
  try {
    // Check if HIPAA mode is enabled
    const hipaaEnabled = await checkHIPAAEnabled(businessId);
    if (!hipaaEnabled) {
      return null; // HIPAA mode not enabled, skip analysis
    }

    // Get compliance settings
    const settings = await getComplianceSettings(businessId);
    if (!settings?.auto_phi_detection) {
      return null; // Auto-detection not enabled
    }

    // Get categories to check
    const categoriesToCheck = settings.phi_detection_categories as PHICategory[] || [];
    ...
    // Detect PHI
    const detectedCategories = detectPHIInText(transcriptText, categoriesToCheck);

    // If PHI detected, flag the call
    if (detectedCategories.length > 0) {
      await markCallAsPHI(callId, detectedCategories, "system");
      logError("PHI Detected in Call", new Error(`Detected categories: ${detectedCategories.join(", ")}`));
    }

    return detectedCategories;
  } catch (error) {
    logError("PHI Analysis Error", error);
    return null;
  }
}

/**
 * Process call analysis webhook with PHI detection
 *
 * This is a wrapper that should be called from the Retell webhook handler
 * after call_analyzed event to check for PHI in the transcript.
 *
 * @param callId - Call ID
 * @param businessId - Business ID
 * @param callAnalysis - Call analysis data from Retell
 * @param transcript - Transcript object from Retell
 */
export async function processCallAnalysisWithPHI(
  callId: string,
  businessId: string,
  callAnalysis: {
    call_summary?: string;
  ...
  // Analyze transcript for PHI
  const detectedCategories = await analyzeCallForPHI(callId, businessId, transcript);

  // Also check the call summary for PHI
  if (callAnalysis?.call_summary) {
    const summaryPHI = await analyzeCallForPHI(
      callId,
      businessId,
      callAnalysis.call_summary
    );
    if (summaryPHI && summaryPHI.length > 0) {
      // Merge with transcript detection results
      ...
      return {
        phiDetected: true,
        phiCategories: Array.from(allCategories),
      };
    }
  }

  return {
    phiDetected: (detectedCategories?.length || 0) > 0,
    phiCategories: detectedCategories || [],
  };
}
