...
import { verifyWebhookSignature, getCallDetails, processCallAnalysisWithPHI } from "@/lib/retell";
      ...
      case "call_ended": {
        ...
        if (durationMinutesBilled > 0) {
          ...
          if (isTrialing) {
            ...
            // Regular billing - increment minutes used
      ...
      case "call_analyzed": {
        ...
        if (analysis) {
          ...
          if (existingCall) {
            ...
            // Analyze for PHI content (HIPAA compliance)
            const phiResult = await processCallAnalysisWithPHI(
              existingCall.id,
              businessId,
              analysis,
              call.transcript_object || null
            );

            // Update call record with analysis and PHI detection results
            ...
            // Add PHI detection results if PHI was detected
            if (phiResult.phiDetected) {
              updateData.phi_detected = true;
              updateData.phi_categories = phiResult.phiCategories;
              updateData.phi_flagged_at = new Date().toISOString();
              updateData.phi_flagged_by = "system";
            }
