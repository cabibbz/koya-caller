...
export const GET = withAuth(async (request: NextRequest, { business }) => {
  console.log("[Inbox API] Business ID:", business.id);

  const grant = await getNylasGrant(business.id);
  console.log("[Inbox API] Grant from DB:", grant ? { grantId: grant.grantId, email: grant.grantEmail, provider: grant.grantProvider } : null);
  ...
  try {
    console.log("[Inbox API] Calling listMessages with grantId:", grant.grantId);
