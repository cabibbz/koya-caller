...
async function handleGet(
  ...
  try {
    ...
    // Count successful calls (answered and completed positively)
    const { count: successfulCount } = await anySupabase
      .from("outbound_call_queue")
      .select("*", { count: "exact", head: true })
      .eq("campaign_id", id)
      .eq("status", "completed")
      .in("outcome", ["booked", "transferred", "message_taken", "completed", "info"]);

    // Count declined calls (explicitly rejected by recipient)
    const { count: declinedCount } = await anySupabase
      .from("outbound_call_queue")
      .select("*", { count: "exact", head: true })
      .eq("campaign_id", id)
      .in("status", ["declined"])
      .or("outcome.eq.declined,outcome.eq.rejected");

    // Count unsuccessful calls (no answer, voicemail, busy, error, etc.)
    const { count: unsuccessfulStatusCount } = await anySupabase
      .from("outbound_call_queue")
      .select("*", { count: "exact", head: true })
      .eq("campaign_id", id)
      .in("status", ["failed", "dnc_blocked", "no_answer"]);

    const { count: unsuccessfulOutcomeCount } = await anySupabase
      .from("outbound_call_queue")
      .select("*", { count: "exact", head: true })
      .eq("campaign_id", id)
      .eq("status", "completed")
      .in("outcome", ["no_answer", "voicemail", "busy", "error", "invalid_number"]);
    ...
    const declined = declinedCount || 0;
    const unsuccessful = (unsuccessfulStatusCount || 0) + (unsuccessfulOutcomeCount || 0);
    const inProgress = inProgressCount || 0;
    const completed = successful + declined + unsuccessful;
    ...
    return success({
      ...
      calls_unsuccessful: unsuccessful,
...
async function handlePost(
  ...
  try {
    ...
    // If starting a call campaign, automatically process the queue to begin making calls
    if (body.action === "start" && existingCampaign.type !== "email") {
      try {
        // Import the processing logic dynamically
        const { initiateOutboundCall } = await import("@/lib/outbound");

        // Get pending queue items
        const { data: queueItems } = await adminSupabase
          .from("outbound_call_queue")
          .select("*")
          .eq("campaign_id", id)
          .eq("status", "pending")
        ...
        if (queueItems && queueItems.length > 0) {
          ...
          logInfo("Campaign Start", `Auto-processed ${queueItems.length} calls for campaign ${id}`);
        }
      } catch (processError) {
        // Log but don't fail - campaign is still started
        logError("Campaign Start Auto-Process", processError);
      }
    }
