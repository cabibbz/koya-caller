/**
 * OpenTable OAuth Initiation Route
 *
 * POST /api/integrations/opentable/auth
 * Initiates the OpenTable OAuth flow
 */
  ...
  try {
    ...
    if (!OPENTABLE_CLIENT_ID) {
      return NextResponse.json(
        { error: "OpenTable integration is not configured. Please contact support." },
        { status: 503 }
      );
    }
    ...
    const authUrl = `https://oauth.opentable.com/api/v2/oauth/authorize?` +
      new URLSearchParams({
        client_id: OPENTABLE_CLIENT_ID,
        scope: OPENTABLE_SCOPES,
    ...
    response.cookies.set("opentable_oauth_state", state, {
      httpOnly: true,
    ...
    logError("OpenTable Auth", error);
    return NextResponse.json(
      { error: "Failed to initiate OpenTable connection" },
      { status: 500 }
    );
  }
}
