/**
 * Mindbody OAuth Callback Route
 *
 * GET /api/integrations/mindbody/callback
 * Handles the OAuth callback from Mindbody
 */
...
const MINDBODY_CLIENT_SECRET = process.env.MINDBODY_CLIENT_SECRET;
...
function parseOAuthState(state: string): {
  ...
  try {
    ...
    if (!parsed.businessId || !parsed.returnUrl || !parsed.nonce || !parsed.siteId) {
      return null;
    }
...
export async function GET(request: NextRequest) {
  ...
  const storedState = request.cookies.get("mindbody_oauth_state")?.value;
  ...
  // Parse state to get business ID, return URL, and site ID
  ...
  const { businessId, returnUrl, siteId } = stateData;
  ...
  try {
    ...
    const tokenResponse = await fetch("https://auth.mindbodyonline.com/connect/token", {
      method: "POST",
      headers: {
    ...
    if (dbError) {
      throw new Error("Failed to save Mindbody connection");
    }

    // Clear the state cookie
    const response = NextResponse.redirect(
      `${baseUrl}${safeReturnUrl}${safeReturnUrl.includes("?") ? "&" : "?"}success=${encodeURIComponent("Mindbody connected successfully!")}`
    );
    response.cookies.delete("mindbody_oauth_state");

    return response;
  } catch (error) {
    logError("Mindbody Callback", error);
    const errorMessage =
      error instanceof Error ? error.message : "Failed to connect Mindbody";
    return NextResponse.redirect(
