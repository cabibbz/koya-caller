...
async function handlePost(request: NextRequest) {
  try {
    ...
    if (!result.success) {
      // Handle consent requirement error specifically
      if (result.reason === "consent_required") {
        return NextResponse.json(
          {
            error: result.error || "Patient consent is required before making this call",
            reason: result.reason,
            missing_consents: result.missingConsents,
            help: "The contact must provide consent for voice_recording and ai_processing before outbound calls can be made in HIPAA mode.",
          },
          { status: 403 }
        );
      }
